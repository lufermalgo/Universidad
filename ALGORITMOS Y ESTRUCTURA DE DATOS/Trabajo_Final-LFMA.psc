...
SubProceso inicializacionCero(Matriz,longMatirz)
	Escribir cuadro;
	Para i <- 1 Hasta longMatirz Con Paso 1 Hacer
		Para j <- 1 Hasta longMatirz Con Paso 1 Hacer
			Matriz[i,j] <- 0;
		FinPara
	FinPara
FinSubProceso
...
SubProceso longMatirz<-LeerDato(msn)
    Repetir		
		Escribir Sin Saltar msn
		Leer longMatirz; 
		Limpiar Pantalla;
		Si longMatirz <= 11 Entonces		
			Si longMatirz Es Multiplo De 2 Entonces
	...
	Hasta Que longMatirz <= 11 Y longMatirz Es Impar
FinSubProceso

// identificamos el centro de la matiz.
SubProceso posC<-calcularCentro(longMatirz)
	posC <- redon(longMatirz / 2);
FinSubProceso

// llenamos la matriz, iniciamos con la posición Fila 1 Columna 2, valor 1.
SubProceso crearVector(Matriz,longMatirz,cuadro,posF,posC,checkPoingFila,checkPointColumna)
	Para i <- 1 Hasta cuadro Con Paso 1 Hacer
		// se valida el valor de las variables posF y posC para que no superen el limite.
		si posF < 1 Entonces 
			posF <- longMatirz;
		FinSi	
		Si posC > longMatirz Entonces
			posC <- 1;						
		FinSi
...
Proceso CuadroMagico
	...
	Definir longMatirz,posF,posC,checkPoingFila,checkPointColumna Como Entero;
	...
	longMatirz<-LeerDato("Ingrese un número entre 3 y 11, este debe de ser impar: ");
	// Se define la longitud de la matriz A, para el caso el tamaño maximo de 11.
	Dimension Matriz[longMatirz,longMatirz];
	cuadro <- longMatirz * longMatirz
	inicializacionCero(Matriz,longMatirz);
	posC<-calcularCentro(longMatirz);
	crearVector(Matriz,longMatirz,cuadro,posF,posC,checkPoingFila,checkPointColumna);
	
	//imprimir resultados
	Escribir "";
	Escribir  "El cuadrado magico de (",longMatirz," x ",longMatirz,"):";
	Escribir  "--------------------------------------------------------" ;
	//ciclo para mostrar los resultados de la matiz B
	espacio <- "";
	Para i<-1 Hasta longMatirz Con Paso 1 Hacer
		Para j<-1 Hasta longMatirz Con Paso 1 Hacer
			//se agragan espacios segun el numero, para mejorar la presentacion del cuadrado magico
			espacio <- "";
